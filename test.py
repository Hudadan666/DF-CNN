...
resume = '/home/HDD/project/meeting/DF-CNN/ckpt/only-final-lr-0.01-iter-130000.pth'
folder = '/home/HDD/project/meeting/DF-CNN/results/val'
...
with torch.no_grad():
    for i, (image, ori, img_files) in enumerate(test_loader):
        ...
        outs, df = model(image, (h, w))
        fuse = outs[-1] / df
        fuse = torch.clamp(fuse, 0, 1)[0][0].detach().cpu().numpy()
