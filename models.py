...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.df_conv1 = nn.Conv2d(1, 128, 3, padding=1)
        self.df_conv2 = nn.Conv2d(128, 128, 3, padding=1)
        self.df_conv3 = nn.Conv2d(128, 1, 3, padding=1)
    ...
    def forward(self, x, size):
        ...
        prediction = results[-1].cpu()
        pre_mask = torch.lt(prediction, 0.25).float()
        # print(pre_mask)
        pre_mask = pre_mask.cuda()
        df1 = self.df_conv1(pre_mask)
        df1 = self.relu(df1)
        df2 = self.df_conv2(df1)
        df2 = self.relu(df2)
        df3 = self.df_conv3(df2)
        df3 = 5*torch.sigmoid(df3)
        return results, df3
...
def resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        resnet_101 = torch.load("/home/HDD/project/meeting/DF-CNN/only-final-lr-0.01-iter-230000.pth")
        model.load_state_dict(resnet_101, strict=False)
        # torch_params = model.state_dict()
        # # torch_params = model.state_dict()
        # for k in resnet101.keys():
        #     if k == 'fc.weight' or k == 'fc.bias':
        #         continue
        #     else:
        #         torch_params[k] = resnet101[k]
