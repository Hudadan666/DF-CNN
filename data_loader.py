...
class BSDS_RCFLoader(data.Dataset):
    ...
    Dataloader BSDS500
    """
    def __init__(self, root='/home/HDD/project/meeting/rcf-edge-detection/data/HED-BSDS_PASCAL', split='train', transform=False):
        self.root = root
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            lb[lb >= 64] = 1
            lb1 = lb*1
            where_0 = np.where(lb1 == 0)
            where_1 = np.where(lb1 == 1)
            where_2 = np.where(lb1 == 2)
            lb1[where_0] = 1
            lb1[where_1] = 0
            lb1[where_2] = 1
            mask = ndimage.morphology.distance_transform_edt(lb1)
            where0 = np.where(mask == 0)
            where1 = np.where(mask == 1)
            where_1_2 = np.where(np.logical_and(mask > 1, mask <= 2))
            where_2_3 = np.where(np.logical_and(mask > 2, mask <= 3))
            where_3_4 = np.where(np.logical_and(mask > 3, mask <= 4))
            where_4_end = np.where(mask > 4)
            mask[where0] = 0
            mask[where1] = 1
            mask[where_1_2] = 2
            mask[where_2_3] = 3
